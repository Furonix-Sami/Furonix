...
async function loadOverview() {
  ...
  body.innerHTML = ordersRes.data.slice(0, 6).map(o => `
    <tr class="hover:bg-slate-50">
      <td class="px-6 py-4 font-medium text-blue-600">${escapeHtml(o.orderId)}</td>
      <td class="px-6 py-4">${escapeHtml(o.customerName)}</td>
      <td class="px-6 py-4 font-medium">${escapeHtml(currency)} ${Number(o.total).toLocaleString()}</td>
      <td class="px-6 py-4">${escapeHtml(o.paymentMethod)}</td>
      <td class="px-6 py-4"><span class="px-3 py-1 ${badgeColors[o.status] || 'bg-slate-100 text-slate-600'} rounded-full text-xs font-bold">${escapeHtml(o.status)}</span></td>
    </tr>
  `).join('');
}
...
async function loadProducts() {
  ...
  body.innerHTML = productsCache.map(p => `
    <tr class="hover:bg-slate-50">
      <td class="px-6 py-4 font-medium text-slate-900">${escapeHtml(p.name)}</td>
      <td class="px-6 py-4">${escapeHtml(p.category)}</td>
      <td class="px-6 py-4">Rs. ${Number(p.price).toLocaleString()}</td>
      <td class="px-6 py-4">${escapeHtml(p.stock)}</td>
      <td class="px-6 py-4"><span class="px-3 py-1 ${p.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'} rounded-full text-xs font-bold">${escapeHtml(p.status)}</span></td>
      <td class="px-6 py-4">
        <button onclick="toggleFeatured('${escapeHtml(p.productId)}', ${String(p.featured) === 'true'})" title="Toggle featured" class="text-lg ${String(p.featured) === 'true' ? 'text-yellow-400' : 'text-slate-300'} hover:text-yellow-400 transition-colors">★</button>
      </td>
      <td class="px-6 py-4">
        <button onclick="openProductModal('${escapeHtml(p.productId)}')" class="text-blue-600 font-medium text-sm hover:underline mr-3">Edit</button>
        <button onclick="deleteProductConfirm('${escapeHtml(p.productId)}')" class="text-red-500 font-medium text-sm hover:underline">Delete</button>
      </td>
    </tr>
  `).join('');
}
...
function openProductModal(productId) {
  ...
  const categoryOptions = currentCategories.map(c => `<option value="${escapeHtml(c.name)}" ${p && p.category === c.name ? 'selected' : ''}>${escapeHtml(c.name)}</option>`).join('');
  ...
  document.getElementById('modal-root').innerHTML = `
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onclick="if(event.target===this) closeAdminModal()">
      <div class="bg-white rounded-3xl p-8 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form id="product-form" class="space-y-4" onsubmit="saveProduct(event, ${p ? `'${escapeHtml(p.productId)}'` : 'null'})">
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-2">Name *</label>
            <input name="name" required value="${p ? escapeHtml(p.name) : ''}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500">
          </div>
          ...
          <div>
            ...
            <input type="hidden" name="images" id="product-images-value" value="${p ? escapeHtml(p.images) : ''}">
          </div>
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-2">Description</label>
            <textarea name="description" rows="3" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500">${p ? escapeHtml(p.description) : ''}</textarea>
          </div>
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-2">Specifications (one per line)</label>
            <textarea name="specifications" rows="3" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500">${p ? escapeHtml(p.specifications) : ''}</textarea>
          </div>
...
function renderImagePreviews() {
  ...
  box.innerHTML = urls.map((u, i) => `
    <div class="relative w-16 h-16">
      <img src="${escapeHtml(driveImg(u))}" class="w-16 h-16 object-cover rounded-lg border border-slate-200">
...
async function loadOrders() {
  ...
  body.innerHTML = res.data.map(o => `
    <tr class="hover:bg-slate-50">
      <td class="px-6 py-4 font-medium text-blue-600">${escapeHtml(o.orderId)}</td>
      <td class="px-6 py-4">${escapeHtml(o.customerName)}</td>
      <td class="px-6 py-4">${escapeHtml(o.phone)}</td>
      <td class="px-6 py-4 font-medium">Rs. ${Number(o.total).toLocaleString()}</td>
      <td class="px-6 py-4">${escapeHtml(o.paymentMethod)}</td>
      <td class="px-6 py-4">
        <select onchange="changeOrderStatus('${escapeHtml(o.orderId)}', this.value)" class="bg-slate-50 border border-slate-200 rounded-full px-3 py-1.5 text-xs outline-none">
          ${statuses.map(s => `<option value="${s}" ${s === o.status ? 'selected' : ''}>${s}</option>`).join('')}
        </select>
      </td>
      <td class="px-6 py-4"><button onclick="viewOrder('${escapeHtml(o.orderId)}')" class="text-blue-600 font-medium text-sm hover:underline">View</button></td>
    </tr>
  `).join('');
}
document.addEventListener('DOMContentLoaded', () => {
...
async function viewOrder(orderId) {
  ...
  const itemsHtml = o.items.map(i => `
    <div class="flex justify-between text-sm py-1">
      <span>${escapeHtml(i.productName)} x ${escapeHtml(i.quantity)}</span>
  ...
  document.getElementById('modal-root').innerHTML = `
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onclick="if(event.target===this) closeAdminModal()">
      <div class="bg-white rounded-3xl p-8 w-full max-w-md max-h-[90vh] overflow-y-auto">
        <h3 class="text-xl font-bold text-slate-900 mb-4">Order ${escapeHtml(o.orderId)}</h3>
        <div class="text-sm space-y-1 text-slate-600 mb-4">
          <p><strong class="text-slate-900">Customer:</strong> ${escapeHtml(o.customerName)}</p>
          <p><strong class="text-slate-900">Phone:</strong> ${escapeHtml(o.phone)}</p>
          <p><strong class="text-slate-900">Email:</strong> ${escapeHtml(o.email) || '-'}</p>
          <p><strong class="text-slate-900">Address:</strong> ${escapeHtml(o.address)}, ${escapeHtml(o.city)}</p>
          <p><strong class="text-slate-900">Payment:</strong> ${escapeHtml(o.paymentMethod)}</p>
        </div>
...
async function loadCategories() {
  ...
  list.innerHTML = currentCategories.map(c => `
    <li class="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
      ...
      <div>
        <button onclick="openCategoryModal('${escapeHtml(c.categoryId)}')" class="text-sm text-blue-600 font-medium hover:underline mr-3">Edit</button>
        <button onclick="deleteCategoryConfirm('${escapeHtml(c.categoryId)}')" class="text-sm text-red-500 font-medium hover:underline">Delete</button>
      </div>
    </li>
  `).join('');
}
  ...
  document.getElementById('modal-root').innerHTML = `
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onclick="if(event.target===this) closeAdminModal()">
      <div class="bg-white rounded-3xl p-8 w-full max-w-sm">
        ...
        <form class="space-y-4" onsubmit="saveCategory(event, ${c ? `'${escapeHtml(c.categoryId)}'` : 'null'})">
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-2">Name *</label>
            <input name="name" required value="${c ? escapeHtml(c.name) : ''}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500">
          </div>
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-2">Status</label>
            <select name="status" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500">
